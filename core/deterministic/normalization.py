...
class DataNormalizer:
    ...
    def normalize(self, df: pd.DataFrame) -> NormalizationResult:
        ...
        df_normalized, type_transformations, type_warnings = (
            self._detect_and_coerce_types(df_normalized)
        )
        ...
        period_key = time_detector.compose_period_key(
            df_normalized, time_info["period_grain"], time_info["derivations"]
        )
        ...
        for col in df.columns:
            ...
            if numeric_counters["successful_coercions"] > 0:
                ...
                if numeric_counters.get("decimal_convention") == "mixed":
                    warnings.append(
                        f"Column '{col}' contains mixed decimal conventions (both US and EU formats)"
                    )
    ...
    def _coerce_numeric(
        ...
        if len(counters["decimal_conventions_seen"]) == 0:
            ...
            counters["decimal_convention"] = list(counters["decimal_conventions_seen"])[
                0
            ]
        else:
        ...
        counters["decimal_conventions_seen"] = list(
            counters["decimal_conventions_seen"]
        )
    ...
    def _normalize_percentages(
        ...
        counters = {"percent_normalized": 0, "representation": None}
    ...
    def _coerce_datetime(
        ...
        counters = {"datetime_parsed": 0, "parsing_errors": 0}
    ...
    def _apply_domain_rules(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        negative_allowed = (
            financial_negative_allowed + settings.negative_allowed_columns
        )

        warnings = []
        anomalies = {}
        metadata = {"multi_currency_columns": [], "mixed_decimal_conventions": []}
            ...
            if df[col].dtype in ["float64", "int64"]:
                ...
                if neg_count > 0:
                    anomalies[col] = {
                        ...
                        "negative_rate": (
                            float(neg_count / total_count) if total_count > 0 else 0.0
                        ),
                    }

                    # Flag negative revenues and other unexpected negatives
                    is_negative_allowed = any(
                        allowed in col.lower() for allowed in negative_allowed
                    )
        ...
        return {"warnings": warnings, "anomalies": anomalies, "metadata": metadata}
                ...
                if len(numeric_data) > 0:
                    ...
                    if std_val > 0:
                        outliers = numeric_data[
                            np.abs(numeric_data - mean_val) > 3 * std_val
                        ]
                        if len(outliers) > 0:
                            col_anomalies["outlier_count"] = len(outliers)
                            col_anomalies["outlier_rate"] = float(
                                len(outliers) / len(numeric_data)
                            )
    ...
    def _generate_schema(
        ...
        for col in df.columns:
            ...
            if df[col].dtype in ["float64", "int64"]:
                ...
                role = (
                    "numeric"
                    if cardinality > threshold or cardinality > total_rows * 0.5
                    else "categorical"
                )
            ...
            col_info = {
                ...
                "coercions": {
                    ...
                    "parentheses_to_negative": col_coercions.get(
                        "parentheses_to_negative", 0
                    ),
                    "scaling_applied": col_coercions.get("scaling_applied", 0),
                    "unicode_minus_normalized": col_coercions.get(
                        "unicode_minus_normalized", 0
                    ),
                    ...
                    "failed_numeric_coercions": col_coercions.get(
                        "failed_numeric_coercions", 0
                    ),
                },
        ...
        schema = {
            ...
            "metadata": {
                ...
                "has_time_dimension": any(
                    col["role"] == "datetime" for col in columns_info
                ),
            },
            "warnings": warnings,
            "transformations_summary": {
                ...
                "transformation_types": list(
                    set(t["step"] for t in transformations if "step" in t)
                ),
            },
        }
