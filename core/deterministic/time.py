...
class TimeDetector:
    ...
    _QUARTER_PATTERN = re.compile(
        r"\b(quarter|qtr|q[1-4]|trimestre|trim)\b", re.IGNORECASE
    )
    _DATE_PATTERN = re.compile(
        r"\b(date|dt|fecha|data|datetime|timestamp)", re.IGNORECASE
    )
    ...
    def detect_time_dimensions(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        """
        warnings = []
        time_candidates = {"date": [], "year": [], "month": [], "quarter": []}
        ...
        return {
            ...
            "time_candidates": [
                col for cols in time_candidates.values() for col in cols
            ],
    ...
    def compose_period_key(
        self, df: pd.DataFrame, period_grain: str, derivations: Dict[str, Any]
    ) -> pd.Series:
        """
            ...
            if period_grain == "date":
                ...
                if year_col and quarter_col:
                    ...
                    if valid_mask.any():
                        ...
                        result.loc[valid_mask] = (
                            valid_years.astype(str) + "-Q" + valid_quarters.astype(str)
                        )
    ...
    def _determine_period_grain(
        self, candidates: Dict[str, List], warnings: List[str]
        ...
        if candidates["date"]:
            if len(candidates["date"]) > 1:
                warnings.append(
                    f"Multiple date columns found: {candidates['date']}. Using first: {candidates['date'][0]}"
                )
            ...
            if len(candidates["year"]) > 1:
                warnings.append(
                    f"Multiple year columns found: {candidates['year']}. Using first."
                )
            if len(candidates["month"]) > 1:
                warnings.append(
                    f"Multiple month columns found: {candidates['month']}. Using first."
                )
            ...
            if len(candidates["quarter"]) > 1:
                warnings.append(
                    f"Multiple quarter columns found: {candidates['quarter']}. Using first."
                )
            ...
            if len(candidates["year"]) > 1:
                warnings.append(
                    f"Multiple year columns found: {candidates['year']}. Using first: {candidates['year'][0]}"
                )
            selected_columns["year"] = candidates["year"][0]
            derivations["year_column"] = candidates["year"][0]
    ...
    def _validate_year_column(self, series: pd.Series) -> bool:
        ...
        try:
            ...
            valid_years = numeric_values[
                (numeric_values >= settings.year_range_min)
                & (numeric_values <= settings.year_range_max)
            ]
            return (
                len(valid_years) >= len(non_null) * settings.time_validation_threshold
            )
        except (ValueError, TypeError, AttributeError):
            return False
    ...
    def _validate_month_column(self, series: pd.Series) -> bool:
        ...
        return (
            valid_count >= min(10, len(non_null)) * settings.time_validation_threshold
        )
