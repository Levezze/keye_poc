...
class PromptBuilder:
    ...
    IDENTIFIER_PATTERNS = [
        r'.*id$', r'.*_id$', r'.*identifier.*', r'.*email.*',
        r'.*phone.*', r'.*ssn.*', r'.*account.*', r'.*customer_no.*'
    ]
    ...
    @classmethod
    def _sanitize_sample_data(cls, sample_data: List[Dict[str, Any]], max_items: int = 10) -> List[Dict[str, Any]]:
        ...
        for item in limited_data:
            ...
            for key, value in item.items():
                ...
                sanitized_value = cls._redact_pii(str(value)) if value is not None else None
            ...
            if key == 'schema' and isinstance(value, dict):
                ...
                if 'columns' in sanitized_schema:
                    ...
                    for col in sanitized_schema['columns']:
                        ...
                        sanitized_col['name'] = cls._sanitize_column_name(col.get('name', ''))
                        # Redact from any text fields
                        for text_field in ['original_name', 'notes']:
                            if text_field in sanitized_col:
                                sanitized_col[text_field] = cls._redact_pii(str(sanitized_col[text_field]))
                ...
                if 'by_period' in sanitized_analysis:
                    ...
                    for period_data in sanitized_analysis['by_period']:
                        ...
                        if 'head' in sanitized_period:
                            sanitized_period['head'] = cls._sanitize_sample_data(sanitized_period['head'])
                ...
                if isinstance(value, str):
                    ...
                    sanitized_context[key] = cls._redact_pii(json.dumps(value, default=str))
                else:
    ...
    def _validate_user_question(cls, question: str) -> str:
        ...
        for pattern in suspicious_patterns:
            if re.search(pattern, question, re.IGNORECASE):
                raise PromptSecurityError(f"Potentially unsafe user input detected: {pattern}")
...
def build_schema_description_prompt(
    ...
    return [
        {
            "role": "system",
            "content": SYSTEM_PROMPTS["schema_description"]
        },
        {
            "role": "user", 
...
def build_narrative_insights_prompt(
    ...
    return [
        {
            "role": "system",
            "content": SYSTEM_PROMPTS["narrative_insights"]
        },
        {
            "role": "user",
...
def build_risk_flags_prompt(concentration_results: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    return [
        {
            "role": "system",
            "content": SYSTEM_PROMPTS["risk_flags"]
        },
        {
            "role": "user",
...
def build_data_quality_prompt(
    ...
    return [
        {
            "role": "system", 
            "content": SYSTEM_PROMPTS["data_quality_report"]
        },
        {
            "role": "user",
...
def build_threshold_recommendations_prompt(
    ...
    return [
        {
            "role": "system",
            "content": SYSTEM_PROMPTS["threshold_recommendations"]
        },
        {
            "role": "user",
...
def build_qa_prompt(
    ...
    return [
        {
            "role": "system",
            "content": SYSTEM_PROMPTS["qa_over_context"]
        },
        {
            "role": "user",
...
PROMPT_BUILDERS = {
    "schema_description": build_schema_description_prompt,
    "narrative_insights": build_narrative_insights_prompt,
    "risk_flags": build_risk_flags_prompt,  
    "data_quality_report": build_data_quality_prompt,
    "threshold_recommendations": build_threshold_recommendations_prompt,
    "qa_over_context": build_qa_prompt
}