...
class LLMStatus(BaseModel):
    ...
    reason: Optional[str] = Field(
        default=None,
        description="Reason for status (e.g., 'disabled', 'cached', 'error')",
    )
...
class SchemaDescription(BaseModel):
    ...
    data_quality_notes: List[str] = Field(
        ...
        examples=[
            "High null rate in optional_field column",
            "Some negative values in revenue column",
        ],
    )
    recommended_analyses: List[str] = Field(
        description="Suggested analytical approaches based on schema",
        examples=[
            "Time series analysis on transaction dates",
        ...
        json_schema_extra={
            "example": {
                ...
                "data_quality_notes": [
                    ...
                    "Some outliers in transaction amounts",
                ],
...
class NarrativeInsights(BaseModel):
    ...
    opportunities: List[str] = Field(
        description="Business opportunities suggested by the data", max_length=5
    )
...
class RiskFlags(BaseModel):
    ...
    reasons: List[str] = Field(
        description="Specific reasons for the risk level", min_length=1, max_length=5
    )
    score: Optional[float] = Field(
        default=None, description="Numerical risk score (0-100)", ge=0, le=100
    )
...
class DataQualityReport(BaseModel):
    ...
    issues: List[str] = Field(
        description="Identified data quality issues", max_length=10
    )
    recommendations: List[str] = Field(
        description="Recommended actions to improve data quality", max_length=10
    )
...
class ThresholdRecommendations(BaseModel):
    ...
    suggested: List[int] = Field(
        description="Suggested threshold percentages", min_length=1, max_length=5
    )
    rationale: str = Field(
...
class QAOverContext(BaseModel):
    ...
    confidence: Optional[str] = Field(
        default=None, description="Confidence level in the answer (high/medium/low)"
    )
...
def export_schemas() -> Dict[str, Dict[str, Any]]:
    ...
    return {name: get_json_schema(model) for name, model in LLM_FUNCTION_MODELS.items()}
