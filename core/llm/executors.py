...
class LLMExecutor:
    ...
    def _get_fallback_response(self, function_name: str) -> Dict[str, Any]:
        ...
        fallbacks = {
            ...
            "risk_flags": {
                "level": "medium",
                "reasons": [
                    "Unable to assess risk level - please review concentration metrics manually"
                ],
            },
            "data_quality_report": {
                "issues": [],
    ...
    async def generate_full_insights(
        ...
        tasks = [
            self.generate_narrative_insights(
                dataset_id, concentration_results, schema, thresholds, request_id, model
            ),
            self.generate_risk_flags(
                dataset_id, concentration_results, request_id, model
            ),
            self.generate_threshold_recommendations(
                dataset_id, concentration_results, thresholds, request_id, model
            ),
        ]
        ...
        try:
            ...
            narrative, narrative_status = (
                ...
                else (None, LLMStatus(used=False, reason="error"))
            )
            risk_flags, risk_status = (
                results[1]
            ...
            return {
                "narrative_insights": {
                    "data": (
                        ...
                        if narrative
                        else self._get_fallback_response("narrative_insights")
                    ),
                    "llm_status": narrative_status.model_dump(),
                },
                "risk_assessment": {
                    "data": (
                        risk_flags.model_dump()
                        if risk_flags
                        else self._get_fallback_response("risk_flags")
                    ),
                ...
                "threshold_recommendations": {
                    "data": (
                        ...
                        if thresholds
                        else self._get_fallback_response("threshold_recommendations")
                    ),
                ...
                "overall_llm_status": (
                    ...
                    if any(
                        s.used
                        for s in [narrative_status, risk_status, threshold_status]
                    )
                    else "fallback"
                ),
            }
            ...
            return {
                "narrative_insights": {
                    ...
                    "llm_status": LLMStatus(
                        used=False, reason=f"execution_error: {str(e)}"
                    ).model_dump(),
                },
                "risk_assessment": {
