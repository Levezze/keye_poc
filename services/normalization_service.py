...
class NormalizationService:
    ...
    def normalize_and_persist(
        self, dataset_id: str, df: pd.DataFrame, original_filename: str
    ) -> Dict[str, Any]:
        """
        ...
        lineage_step = self.registry.append_lineage_step(
            ...
            metrics={
                ...
                "columns_modified": result.schema["transformations_summary"][
                    "columns_modified"
                ],
                "transformation_types": result.schema["transformations_summary"][
        ...
        if schema:
            ...
            if missing_keys:
                ...
                validation_results["errors"].append(
                    f"Schema missing required keys: {missing_keys}"
                )
            ...
            if "columns" in schema:
                for i, col in enumerate(schema["columns"]):
                    ...
                    missing_col_keys = [
                        key for key in required_col_keys if key not in col
                    ]
        ...
        if dataset_state["has_normalized"]:
            try:
                ...
                if df is None or df.empty:
                    validation_results["is_valid"] = False
                    validation_results["errors"].append(
                        "Normalized data is empty or unreadable"
                    )
                else:
                    validation_results["warnings"].append(
                        f"Normalized data contains {len(df)} rows, {len(df.columns)} columns"
                    )
            except Exception as e:
                validation_results["is_valid"] = False
                validation_results["errors"].append(
                    f"Error reading normalized data: {str(e)}"
                )
