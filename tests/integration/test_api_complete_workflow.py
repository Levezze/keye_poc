...
class TestCompleteAPIWorkflow:
    ...
    def create_test_excel(self) -> bytes:
        ...
        data = {
            ...
            "quarter": [1, 2, 3, 1, 2],
            "customer": ["ACME Corp", "TechStart Inc", "Global Ltd", "ACME Corp", "NewCo"],
    ...
    def test_complete_csv_workflow(self):
        ...
        assert csv_download_response.headers["content-type"] == "text/csv; charset=utf-8"
    ...
    def test_complete_excel_workflow(self):
        ...
        upload_response = self.client.post(
            "/api/v1/upload",
            files={"file": ("test_data.xlsx", excel_content, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")},
            headers=self.headers
        )
        ...
        assert schema_data["period_grain"] == "year_quarter"  # Should detect year+quarter
    ...
    def test_no_time_dimension_workflow(self):
        ...
        assert "No temporal dimension detected" in " ".join(schema_data.get("warnings", []))
